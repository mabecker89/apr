...
# Attach packages:
pacman::p_load(readr, readxl, dplyr, tidyr, janitor, lubridate, stringr)
...
df_apr <- read_excel("./data/base/df_corrected_grid2020_R1_R2_bisonbison.xlsx",
                     sheet = "df_corrected_grid2020_R1_R2_bis") %>%
  clean_names() %>%
...
dep <- df_durations$deployment_id
...
df_tt_none <- df_durations %>%
  ...
  left_join(df_durations, by = c("deployment_id", "duration_days")) %>%
...
df_density_apr <- df_density %>%
  filter(!density_km2 == "NaN",
         ...
         duration_days > 5) %>%
  summarise_density(species_col = species,
